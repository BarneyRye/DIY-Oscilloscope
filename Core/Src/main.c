...
/* USER CODE BEGIN PV */
  ...
  volatile int8_t Trig_Voltage = 0; //Set Initial Trigger Voltage
  volatile uint8_t Trig_Flag = 0;
  uint16_t Last_ADC_Trig = 0;
...
void HAL_ADC_LevelOutOfWindowCallback(ADC_HandleTypeDef* hadc) {
	if (hadc->Instance == ADC1) {
		//Trigger
		uint16_t current_ADC = HAL_ADC_GetValue(hadc);
		if (current_ADC >= Trig_ADC_Value && Last_ADC_Trig < Trig_ADC_Value) {
		Trig_Flag = 1;
		}
...
void ADC_SetTrigger(void) {
	...
	HAL_ADC_AnalogWDGConfigTypeDef awdConfig;
